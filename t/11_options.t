...
EOF
is( CommonMark->markdown_to_html( $md, OPT_SOURCEPOS ), $expected,
    'SOURCEPOS' );

$md = "a\nb";
is( CommonMark->markdown_to_html( $md, OPT_HARDBREAKS ),
    "<p>a<br />\nb</p>\n", 'HARDBREAKS' );
SKIP: {
    skip( 'NOBREAKS not supported by libcmark', 1 )
      if CommonMark->version < 0x001A00;
    is( CommonMark->markdown_to_html( $md, OPT_NOBREAKS ),
        "<p>a b</p>\n", 'NOBREAKS' );
}
is( CommonMark->markdown_to_html($md),
...
is( CommonMark->markdown_to_html( $md, OPT_SAFE | OPT_UNSAFE ),
...
$md = "a\xC0b";
Encode::_utf8_on($md);
is( CommonMark->markdown_to_html( $md, OPT_VALIDATE_UTF8 ),
    "<p>a\x{FFFD}b</p>\n", 'VALIDATE_UTF8' );
