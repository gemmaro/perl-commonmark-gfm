...
CommonMark::GFM::Node - Node of the GitHub Flavored Markdown parse tree
...
=head1 SYNOPSIS
    ...
    my $link = CommonMark::GFM::Node->new(CommonMark::GFM::NODE_LINK);
    $link->set_url('http://example.com/');
    my $text = CommonMark::GFM::Node->new(CommonMark::GFM::NODE_TEXT);
...
C<CommonMark::GFM::Node> represents a node of the parse tree.

=head2 new

   my $node = CommonMark::GFM::Node->new($type);

Creates a new node of type C<$type>. See L</"Node types"> for a list of
types. Note that the L<node creation functions|CommonMark::GFM/"Node creation">
...
=head3 C<render_*>
    ...
    my $text  = $node->render_plaintext( [$options], [$width] )
...
Render options can be imported from L<CommonMark::GFM> with tag C<opt>.
...
Translates "softbreak" nodes to spaces.
...
Deprecated.  Replaces raw HTML with a placeholder HTML comment. This
option is the default and has no effect except to override
C<OPT_UNSAFE>.

=back

See the documentation of I<libcmark-gfm> for a more detailed explanation of the
render options.
...
=head2 Accessors
    ...
    # Others
    my $fenced = $node->get_fenced(my $length, $offset, $character);
    $node->set_fenced($fenced, $length, $offset, $character);
...
Please be warned that C<get_fenced> and C<set_fenced> methods might
not work well for some reason in cmark-gfm, e.g. it seems that it
doesn't get length and offset correctly.

=head2 Tree traversal

    my $iterator = $node->iterator;

Creates a new L<CommonMark::GFM::Iterator> to walk through the descendants of
the node.
    ...
    my $def    = $node->parent_footnote_def;

These methods return the respective node in the tree structure.

C<parent_footnote_def> returns the footnote reference of node, or NULL
if node doesn't have a footnote reference.
...
=head3 Node types
    ...
    CommonMark::GFM::NODE_NONE => 0

    CommonMark::GFM::NODE_DOCUMENT
    CommonMark::GFM::NODE_BLOCK_QUOTE
    CommonMark::GFM::NODE_LIST
    CommonMark::GFM::NODE_ITEM
    CommonMark::GFM::NODE_CODE_BLOCK
    CommonMark::GFM::NODE_HTML_BLOCK
    CommonMark::GFM::NODE_CUSTOM_BLOCK
    CommonMark::GFM::NODE_PARAGRAPH
    CommonMark::GFM::NODE_HEADING
    CommonMark::GFM::NODE_THEMATIC_BREAK
    CommonMark::GFM::FOOTNOTE_DEFINITION

    CommonMark::GFM::NODE_TEXT
    CommonMark::GFM::NODE_SOFTBREAK
    CommonMark::GFM::NODE_LINEBREAK
    CommonMark::GFM::NODE_CODE
    CommonMark::GFM::NODE_HTML_INLINE
    CommonMark::GFM::NODE_CUSTOM_INLINE
    CommonMark::GFM::NODE_EMPH
    CommonMark::GFM::NODE_STRONG
    CommonMark::GFM::NODE_LINK
    CommonMark::GFM::NODE_IMAGE
    CommonMark::GFM::FOOTNOTE_REFERENCE

Node types can be imported from L<CommonMark::GFM> with tag C<node>.

    use CommonMark::GFM qw(:node);

=head3 List types

    CommonMark::GFM::NO_LIST => 0
    CommonMark::GFM::BULLET_LIST
    CommonMark::GFM::ORDERED_LIST

List types can be imported from L<CommonMark::GFM> with tag C<list>.

    use CommonMark::GFM qw(:list);

=head3 Delimiter types for ordered lists

    CommonMark::GFM::NO_DELIM => 0
    CommonMark::GFM::PERIOD_DELIM
    CommonMark::GFM::PAREN_DELIM

Delimiter types can be imported from L<CommonMark::GFM> with tag C<delim>.

    use CommonMark::GFM qw(:delim);

=head1 COPYRIGHT

This software is copyright (C) by Nick Wellnhofer
and copyright (C) 2026 gemmaro.
