...
CommonMark::GFM::Iterator - Iterate GitHub Flavored Markdown nodes
...
C<CommonMark::GFM::Iterator> provides a convenient way to walk through the nodes
in a parse tree.
...
Event types are:
    ...
    CommonMark::GFM::EVENT_DONE
    CommonMark::GFM::EVENT_ENTER
    CommonMark::GFM::EVENT_EXIT

Event types can be imported from L<CommonMark::GFM> with tag C<event>.

    use CommonMark::GFM qw(:event);
...
Enter the node. The event type is C<CommonMark::GFM::EVENT_ENTER> and the current
...
C<CommonMark::GFM::EVENT_EXIT> and the current node is set to the original node
C<V>.

=back

After the root node was exited, the event type is set to
C<CommonMark::GFM::EVENT_DONE> and the current node to C<undef>. In scalar
context, C<next> returns C<CommonMark::GFM::EVENT_DONE>. In list context, it
returns the empty list.

For leaf nodes, no exit events are generated. Leaf nodes comprise the node
types that never have children:

    CommonMark::GFM::NODE_HTML
    CommonMark::GFM::NODE_HRULE
    ...
    CommonMark::GFM::NODE_INLINE_HTML
